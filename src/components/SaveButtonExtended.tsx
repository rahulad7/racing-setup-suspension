...
interface SaveButtonExtendedProps {
  allSetupData: any;
  vehicleId?: string;
  className?: string;
}

const SaveButtonExtended: React.FC<SaveButtonExtendedProps> = ({ 
  allSetupData, 
  vehicleId,
  className = '' 
}) => {
  ...
  // Use current vehicle from context if available, otherwise use passed vehicleId
  ...
  const handleSave = async () => {
    ...
    // Check if user has valid license for saving setups
    if (!isLicenseValid) {
    ...
    setSaving(true);
    try {
      // Extract track name from setup data if available
      const trackName = allSetupData?.track || 
                       allSetupData?.trackData?.name || 
                       allSetupData?.trackData?.trackName || 
                       allSetupData?.track?.name ||
                       allSetupData?.track?.trackName ||
                       null;
      ...
      // Update the current setup in AppContext immediately
      const newSetup = {
        id: data.id,
        name: finalSetupName
      };
      
      console.log('Setting current setup to:', newSetup);
      setCurrentSetup(newSetup);
      
      toast({
        title: 'Setup Saved Successfully',
        description: `"${finalSetupName}" has been saved to your account`,
      });
    ...
    <>
      <div className={`space-y-3 p-4 bg-gray-50 rounded-lg border ${className}`}>
        {selectedVehicleId && (
         ...
         {/* Show vehicle selector only if no current vehicle is selected */}
         {!currentVehicle && (
           <VehicleSelector
             selectedVehicleId={selectedVehicleId}
             onVehicleSelect={() => {}} // Disabled when using context
           />
         )}
...
export default SaveButtonExtended;